...
async function main() {
	...
	try {
			...
			for (const [url, data] of Object.entries(metrics)) {
				...
				for (const [key, value] of Object.entries(data.metrics)) {
					if (typeof value === "number") {
						allMetrics[url].metrics[key] =
							(allMetrics[url].metrics[key] || 0) + value / reportFiles.length;
					}
				}
		...
		for (const [url, data] of Object.entries(allMetrics)) {
			const urlKey =
				url.replace("http://localhost:4321", "").replace(/\/$/, "") ||
				"homepage";
		...
		if (updateMode) {
			...
			for (const [url, data] of Object.entries(allMetrics)) {
				...
				for (const [key, value] of Object.entries(data.metrics)) {
					if (typeof value === "number") {
						const formatted = key.includes("score")
							? value.toFixed(2)
				...
				console.log(
					`  Resource sizes: ${rs.totalKB.toFixed(1)}KB (JS: ${rs.totalJsKB.toFixed(1)}KB, CSS: ${rs.totalCssKB.toFixed(1)}KB)`,
				);
			}
